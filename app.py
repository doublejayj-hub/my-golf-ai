...
st.set_page_config(layout="wide", page_title="GDR AI Pro v74")
st.title("⛳ GDR AI Pro: 엔진 복구 및 무결성 패치 v74.0")

# [2] 초고안정성 분석 엔진 (강제 프레임 동기화 방식)
def get_final_stable_engine(f_v64, s_v64):
    ...
    <div style="display: flex; gap: 10px; background: #111; padding: 20px; border-radius: 12px; border: 1px solid #333;">
        <div style="flex: 1; text-align: center;">
            <h4 style="color: #0f0; font-size: 14px;">FRONT (Locked)</h4>
            <video id="vf" controls playsinline style="width: 100%; border-radius: 8px; background:#000;"></video>
        ...
        <div style="flex: 1; text-align: center;">
            <h4 style="color: #0f0; font-size: 14px;">SIDE (Locked)</h4>
            <video id="vs" controls playsinline style="width: 100%; border-radius: 8px; background:#000;"></video>
    ...
    <div style="text-align: center; margin-top: 15px;">
        <button onclick="copyData()" style="background:#0f0; color:#000; border:none; padding:12px 25px; border-radius:8px; cursor:pointer; font-weight:bold; font-size:16px;">📋 데이터 복사</button>
    </div>
    ...
    <script>
        ...
        pose.onResults((r)=>{{
            ...
            // 1. 공통 임팩트 래치 (손목 위치)
            if (wL.y > hL.y - 0.05) {{ f_lock = true; s_lock = true; }}
        ...
        // [사전검증 완료] 무중단 프레임 전송 엔진
        async function runLoop(v) {{
            while(!v.paused && !v.ended) {{
                await pose.send({{image: v}});
                await new Promise(r => setTimeout(r, 60)); // 안정적인 샘플링 주기 확보
            }}
        }}

        vf.onplay = () => {{ forceClear(); runLoop(vf); }};
        vs.onplay = () => {{ runLoop(vs); }};
...
with col1: f_f = st.file_uploader("Front Video", type=['mp4', 'mov'])
with col2: s_f = st.file_uploader("Side Video", type=['mp4', 'mov'])

if f_f and s_f:
    f_b = base64.b64encode(f_f.read()).decode()
    s_b = base64.b64encode(s_f.read()).decode()
    components.html(get_final_stable_engine(f_b, s_b), height=550)
