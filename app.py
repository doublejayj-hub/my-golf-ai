...
st.set_page_config(layout="wide", page_title="GDR AI Pro v77")
st.title("⛳ GDR AI Pro: 다이렉트 데이터 브릿지 v77.0")

# [2] 초정밀 분석 엔진 (필터 해제 및 강제 출력)
def get_direct_engine(f_v64, s_v64):
    return f"""
    <div style="display: flex; gap: 10px; background: #000; padding: 15px; border-radius: 12px; border: 1px solid #333;">
        <div style="flex: 1; text-align: center;">
            <h4 style="color: #0f0;">FRONT</h4>
    ...
    <script>
        ...
        pose.onResults((r)=>{{
            if(!r.poseLandmarks) {{ log.innerText = "상태: 관절 인식 실패(인물 확인 필요)"; return; }}
            log.innerText = "상태: 데이터 추출 중...";
        ...
        vf.onplay = () => {{ f_c=0; f_pkSw=0; f_pkXF=0; startAnalysis(vf); }};
        vs.onplay = () => {{ s_c=0; s_pkSp=0; s_pkKn=0; startAnalysis(vs); }};
...
if f_f and s_f:
    ...
    components.html(get_direct_engine(f_b, s_b), height=550)
