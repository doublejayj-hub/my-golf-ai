...
st.set_page_config(layout="wide", page_title="GDR AI Pro v72")
st.title("⛳ GDR AI Pro: 임팩트 래치 및 구간 필터링 v72.0")

# [2] 고정밀 구간 엔진 (임팩트 시점 고정 로직)
def get_gated_engine(f_v64, s_v64):
    return f"""
    <div style="display: flex; gap: 10px; background: #000; padding: 20px; border-radius: 12px; border: 1px solid #333;">
        <div style="flex: 1; text-align: center;">
            <h4 style="color: #0f0;">FRONT (Impact Locked)</h4>
            <video id="vf" controls playsinline style="width: 100%; border-radius: 8px;"></video>
            <div style="margin-top:10px; color:#0f0; font-family:monospace; font-size:16px; background:rgba(0,255,0,0.1); padding:10px; border-radius:5px;">
                Max Sway: <span id="f_sw">0.0</span>% | X-Factor: <span id="f_xf">0.0</span>°
            </div>
        </div>
        <div style="flex: 1; text-align: center;">
            <h4 style="color: #0f0;">SIDE (Impact Locked)</h4>
    ...
    <script>
        ...
        poseF.onResults((r)=>{{
            ...
            // [핵심] 임팩트 트리거: 양 손목 중 하나라도 골반 높이(23번) 아래로 내려가면 즉시 잠금
            if (wL.y > hL.y - 0.05 || wR.y > hR.y - 0.05) {{ f_lock = true; return; }}

            if(f_c < 15) {{ f_refH = Math.abs(hL.x - hR.x); f_startCX = (hL.x + hR.x)/2; f_c++; }} 
            else {{
                // Sway 필터링: 백스윙 방향(x좌표 증가)만 측정하고 15% 이상 튀는 노이즈 방어
        ...
        vf.onplay = async () => {{ 
            forceResetAll(); 
            while(!vf.paused && !f_lock){{ await poseF.send({{image:vf}}); await new Promise(r=>setTimeout(r,40)); }} 
        }};
        vs.onplay = async () => {{ 
            while(!vs.paused && !s_lock){{ await poseS.send({{image:vs}}); await new Promise(r=>setTimeout(r,40)); }} 
        }};
...
if f_f and s_f:
    ...
    components.html(get_gated_engine(f_b, s_b), height=550)

st.divider()
in_text = st.text_area("데이터를 붙여넣으세요.")
if st.button("🚀 종합 분석 시작") and model:
    st.write(model.generate_content(f"역학 전문가로서 분석: {in_text}").text)
