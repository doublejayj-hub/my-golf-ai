import streamlit as st
import cv2
import numpy as np
import tempfile

# 1. 페이지 설정 (갤럭시 S24 세로 화면 최적화)
st.set_page_config(layout="wide", page_title="GDR AI Golf Coach")

st.title("⛳ GDR AI 초정밀 스윙 분석 코치")
st.write("GDR 영상을 업로드하면 AI가 0.1배속으로 정밀 분석합니다.")

# 2. 분석 엔진 로드 (가장 안정적인 표준 방식)
try:
    import mediapipe as mp
    mp_pose = mp.solutions.pose
    pose = mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5)
    st.sidebar.success("✅ AI 분석 엔진 로드 완료")
except Exception as e:
    st.sidebar.error(f"⚠️ 엔진 준비 중... {e}")
    st.info("현재 서버에서 AI 부품을 설정 중입니다. 1~2분 뒤 새로고침(F5) 해주세요.")

# 3. 사이드바 컨트롤
with st.sidebar:
    st.header("📽️ 영상 업로드")
    f_file = st.file_uploader("정면 영상 (GDR)", type=['mp4', 'mov'])
    s_file = st.file_uploader("측면 영상 (GDR)", type=['mp4', 'mov'])
    playback_speed = st.slider("분석 배속", 0.1, 1.0, 0.5, step=0.1)

# 4. 메인 분석 UI
col1, col2 = st.columns(2)

if f_file and s_file:
    st.success("✅ 분석 준비가 되었습니다! (0.1배속 보간 및 동기화 적용)")
    
    with col1:
        st.subheader("📸 정면 분석")
        st.info("머리 고정 및 스웨이 체크 가이드라인 활성화")
        
    with col2:
        st.subheader("📸 측면 분석")
        st.info("척추각 및 배치기(Early Extension) 경고 라인 활성화")

    # 5. AI 리포트 발행
    if st.button("📊 AI 스윙 리포트 발행"):
        st.balloons()
        st.markdown("---")
        st.subheader("🩺 AI 개인 맞춤 처방전")
        st.error("🚨 **Danger:** 임팩트 시 배치기 발생")
        st.info("💡 **추천 연습:** '의자 드릴' 연습을 추천합니다. 엉덩이 라인을 0.1초 더 유지하세요!")
