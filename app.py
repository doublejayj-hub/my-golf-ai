...
st.set_page_config(layout="wide", page_title="GDR AI Pro v108")
st.title("⛳ GDR AI Pro: 3단 척추 가이드 및 Sway 교정 v108.0")

def get_overlay_engine(f_v64, s_v64):
    return f"""
    <style>
        ...
        canvas {{ position: absolute; top: 15px; left: 15px; width: calc(100% - 30px); height: calc(100% - 100px); pointer-events: none; }}
    ...
    <div class="v-box">
        <h4 style="color:#0f0; margin:0 0 10px 0;">FRONT: Sway 방향 교정</h4>
    ...
    <div class="v-box">
        <h4 style="color:#0f0; margin:0 0 10px 0;">SIDE: 3단 척추 가이드 (어드레스/탑/임팩트)</h4>
    ...
    <script>
        ...
        function onResults(r) {{
            ...
            // 1. 정면 분석 (Sway 로직 반전 교정)
            if (!vf.paused && !f_d.lock) {{
                f_d.c++;
                let curCX = (hL.x + hR.x) / 2;
                if (f_d.c <= 5) f_d.stCX = curCX;
                else {{
                    // [교정] 백스윙 시 CX가 증가하는지 감소하는지 방향 재매칭 (사용자 기준 오른쪽 이동 양수화)
            ...
            if (!vs.paused && !s_d.lock) {{
                ...
                if (s_d.addr) drawSpine(s_d.addr.s, s_d.addr.h, '#00f');
                ...
                if (s_d.topL) drawSpine(s_d.topL.s, s_d.topL.h, '#f00');
                ...
                if(s_d.topP) {{
                    ...
                    if (wR.y > lm[23].y - 0.02) {{
                        s_d.lock = true;
                        s_d.impL = {{s:sC, h:hC}}; // [3] 임팩트 선 (녹색)
                    }}
                }} else if(diff > s_d.pkSp) s_d.pkSp = diff;

                drawSpine(sC, hC, '#0f0', 3); // 현재 척추선
...
if f_file and s_file:
    ...
    components.html(get_overlay_engine(f_b, s_b), height=1500)
