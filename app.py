...
try:
    ...
    st.sidebar.error(f"⚠️ 엔진 준비 중... (잠시 후 새로고침)")
    st.info("현재 서버에서 AI 부품을 설정 중입니다. 1분 뒤 브라우저를 새로고침 해주세요.")
...
with st.sidebar:
    ...
    playback_speed = st.slider("분석 배속 (0.1x ~ 1.0x)", 0.1, 1.0, 0.5, step=0.1)
...
if f_file and s_file:
    ...
    if st.button("📊 AI 스윙 리포트 및 처방전 발행"):
        ...
        st.write("리포트를 캡처하여 갤러리에 저장하고 친구들에게 공유하세요!")
